...
// ==========================================
const BASE_PRICES = {
    morning: { original: 19000, sub: 14900, name: "Morning Breath (아침)" },
    work: { original: 19000, sub: 14900, name: "Work Breath (낮)" },
    night: { original: 19000, sub: 14900, name: "Night Breath (저녁)" },
...
function calculateQuizResult() {
    ...
    if (maxVal === "morning") {
        if (resultTitle) resultTitle.textContent = "Morning Breath 플랜";
        if (resultDesc) resultDesc.textContent = "아침에 기상 시 재채기와 환절기 비염 증상이 가장 강하신 타입이군요. 보리와 루이보스로 구수한 베이스를 잡고, 맥문동과 작두콩 뿌리 엑기스로 아침 목을 건조함 없이 활짝 깨우는 Morning Breath를 추천합니다.";
        if (resultImg) resultImg.src = "assets/morning_breath.png";
        if (resultProductName) resultProductName.textContent = "1. Morning Breath 정기구독";
    } else if (maxVal === "work") {
        if (resultTitle) resultTitle.textContent = "Work Breath 플랜";
        if (resultDesc) resultDesc.textContent = "일과 중 밀폐된 사무실 공기나 건조한 히터 바람으로 코막힘과 목 답답함을 자주 겪는 타입이시군요. 페퍼민트의 시원한 멘톨 성분과 생강의 따스한 기운으로 즉각적인 리프레시와 코 뻥 뚫림을 전하는 Work Breath를 권장합니다.";
        if (resultImg) resultImg.src = "assets/work_breath.png";
        if (resultProductName) resultProductName.textContent = "2. Work Breath 정기구독";
    } else {
        if (resultTitle) resultTitle.textContent = "Night Breath 플랜";
        if (resultDesc) resultDesc.textContent = "하루 내내 자극받은 호흡기 점막 때문에 잠들기 전에 마른기침을 하거나 답답해하는 밤 민감형 타입이시군요. 맥문동과 대추의 촉촉한 점액질 보습 코팅, 그리고 편안한 수면을 유도하는 캐모마일 블렌딩의 Night Breath가 최상의 선택입니다.";
        if (resultImg) resultImg.src = "assets/night_breath.png";
        if (resultProductName) resultProductName.textContent = "3. Night Breath 정기구독";
    }
...
const TOAST_PRODUCTS = [
    { code: "morning", label: "Morning Breath" },
    { code: "work", label: "Work Breath" },
    { code: "night", label: "Night Breath" },
